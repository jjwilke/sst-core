...
std::vector<SSTLibraryInfo>  g_libInfoArray;
...
public:
    ...
    void outputXML(std::ostream& os, TiXmlNode* XMLParentElement);
} g_Outputter;
...
void generateXMLOutputFile()
{
    g_Outputter.outputXML(std::cout, NULL);
}
...
void OverallOutputter::outputXML(std::ostream& UNUSED(os), TiXmlNode* UNUSED(XMLParentElement))
{
    unsigned int            x;
    char                    TimeStamp[32];
...
{
  auto* lib = ELI::InfoDatabase::getLibrary<T>(getLibraryName());
  if (lib){
    os << "Num " << T::ELI_baseName() << "s = " << lib->numEntries() << "\n";
    int idx = 0;
    for (auto& pair : lib->getMap()){
      bool print = printAll || shouldPrintElement(getLibraryName(), pair.first);
      if (print){
        os << T::ELI_baseName() << " " << idx << ": " << pair.first << "\n";
        pair.second->toString(os);
      }
      ++idx;
    }
  } else {
    os << "No " << T::ELI_baseName() << "s\n";
  }
}
...
void SSTLibraryInfo::outputXML(TiXmlElement* XMLLibraryElement)
{
  ...
  if (lib){
    ...
    xmlComment(XMLLibraryElement, "Num %ss = %d", T::ELI_baseName(), numObjects);
    int idx = 0;
    for (auto& pair : lib->getMap()){
    ...
    xmlComment(XMLLibraryElement, "No %ss", T::ELI_baseName());
  }
}

void SSTLibraryInfo::outputHumanReadable(std::ostream& os, int LibIndex)
{
...
void SSTLibraryInfo::outputXML(int LibIndex, TiXmlNode* XMLParentElement)
{
